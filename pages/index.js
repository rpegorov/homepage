import NextLink from 'next/link'
import {
  Link,
  Container,
  Heading,
  Box,
  SimpleGrid,
  Button,
  List,
  ListItem
} from '@chakra-ui/react'
import { ChevronRightIcon } from '@chakra-ui/icons'
import dynamic from 'next/dynamic'
import Paragraph from '../components/paragraph'
import { BioSection, BioYear } from '../components/bio'
import Layout from '../components/layouts/article'
import Section from '../components/section'
import { GridItem } from '../components/grid-item'
import {
  IoLogoInstagram,
  IoLogoGithub,
  IoLogoVk,
  IoLogoYoutube,
  IoLogoLinkedin
} from 'react-icons/io5'
import { FaTelegram } from 'react-icons/fa'
import thumbYouTube from '../public/images/links/chanel-banner.jpeg'
import Image from 'next/image'
import VoxelLoader from '../components/voxel-loader'
import { useLanguage } from '../lib/i18n'

const LazyVoxelDog = dynamic(() => import('../components/voxel'), {
  ssr: false,
  loading: () => <VoxelLoader />
})

const Home = () => {
  const { t } = useLanguage()
  const bioItems = t('home.bio.items')

  return (
    <Layout>
      <Container>
        <LazyVoxelDog />

        <Box display={{ md: 'flex' }}>
          <Box flexGrow={1}>
            <Heading as="h2" variant="page-title">
              {t('home.heading')}
            </Heading>
            <p>{t('home.tagline')}</p>
          </Box>
          <Box
            flexShrink={0}
            mt={{ base: 4, md: 0 }}
            ml={{ md: 6 }}
            textAlign="center"
          >
            <Box
              borderColor="whiteAlpha.800"
              borderWidth={2}
              borderStyle="solid"
              w="100px"
              h="100px"
              display="inline-block"
              borderRadius="full"
              overflow="hidden"
            >
              <Image
                src="/images/rostislav.jpg"
                alt="Profile image"
                width="100"
                height="100"
              />
            </Box>
          </Box>
        </Box>

        <Section delay={0.1}>
          <Heading as="h3" variant="section-title">
            {t('home.work.heading')}
          </Heading>
          <Paragraph>{t('home.work.body')}</Paragraph>
          <Box align="center" my={4}>
            <Button
              as={NextLink}
              href="/works"
              scroll={false}
              rightIcon={<ChevronRightIcon />}
              colorScheme="teal"
            >
              {t('home.work.button')}
            </Button>
          </Box>
        </Section>

        <Section delay={0.2}>
          <Heading as="h3" variant="section-title">
            {t('home.bio.heading')}
          </Heading>
          {Array.isArray(bioItems) &&
            bioItems.map((item, i) => (
              <BioSection key={i}>
                <BioYear>{item.year}</BioYear>
                {item.text}
              </BioSection>
            ))}
        </Section>

        <Section delay={0.3}>
          <Heading as="h3" variant="section-title">
            {t('home.love.heading')}
          </Heading>
          <Paragraph>
            {t('home.love.art')}, {t('home.love.music')},{' '}
            <Link href="https://www.instagram.com/w1sh.master/" target="_blank">
              {t('home.love.photography')}
            </Link>
            , {t('home.love.ml')},{' '}
            <Link href="https://rutube.ru/channel/53860712/" target="_blank">
              {t('home.love.campingVlog')}
            </Link>
          </Paragraph>
        </Section>

        <Section delay={0.3}>
          <Heading as="h3" variant="section-title">
            {t('home.web.heading')}
          </Heading>
          <List>
            <ListItem>
              <Link href="https://github.com/rpegorov" target="_blank">
                <Button
                  variant="ghost"
                  colorScheme="teal"
                  leftIcon={<IoLogoGithub />}
                >
                  @rpegorov
                </Button>
              </Link>
            </ListItem>
            <ListItem>
              <Link href="https://vk.com/w1shmaster" target="_blank">
                <Button
                  variant="ghost"
                  colorScheme="teal"
                  leftIcon={<IoLogoVk />}
                >
                  @w1shmaster
                </Button>
              </Link>
            </ListItem>
            <ListItem>
              <Link href="https://www.youtube.com/@ROSTCAMP" target="_blank">
                <Button
                  variant="ghost"
                  colorScheme="teal"
                  leftIcon={<IoLogoYoutube />}
                >
                  @ROSTCAMP
                </Button>
              </Link>
            </ListItem>
            <ListItem>
              <Link
                href="https://www.instagram.com/w1sh.master/"
                target="_blank"
              >
                <Button
                  variant="ghost"
                  colorScheme="teal"
                  leftIcon={<IoLogoInstagram />}
                >
                  @w1sh.master
                </Button>
              </Link>
            </ListItem>
            <ListItem>
              <Link
                href="https://www.linkedin.com/in/rostislav-egorov-2721b8220/"
                target="_blank"
              >
                <Button
                  variant="ghost"
                  colorScheme="teal"
                  leftIcon={<IoLogoLinkedin />}
                >
                  @Rostislav Egorov
                </Button>
              </Link>
            </ListItem>
            <ListItem>
              <Link href="https://www.t.me/w1shmaster" target="_blank">
                <Button
                  variant="ghost"
                  colorScheme="teal"
                  leftIcon={<FaTelegram />}
                >
                  @w1shmaster
                </Button>
              </Link>
            </ListItem>
          </List>

          <SimpleGrid columns={[1]} gap={6}>
            <GridItem
              href="https://www.youtube.com/@ROSTCAMP"
              title="ROSTCAMP"
              thumbnail={thumbYouTube}
              leftIcon={<IoLogoYoutube />}
            >
              {t('home.web.youtubeCaption')}
            </GridItem>
          </SimpleGrid>
        </Section>
      </Container>
    </Layout>
  )
}

export default Home
